...
#### Phase A 执行结果（2026-07-31，已通过）

- 权威 v8 现场为 `/public/home/wwb/KE_mem/ke-memory-demo/.runs/operational-deepseek-e2e-v8/memory-history.git`，head commit `0e04cd2c0a7b92ebd0e7eeef7b5bde0e1dc8da0a`、parent `e57b99340719e951ce55fb293f1d95b0c1aac7d6`、checkpoint `checkpoint-3ae5f7c1dd0f709cef5c6824`、bundle `bundle-e2e-bfb6a284f8c01153`，2 条 L1、1 条 L2；raw artifact SHA-256 `bfb6a284f8c01153635b9e6e4f35163013f253c16527db351f5a12e9069dd4ab`。v8 原 receipt 已不存在，commit/checkpoint 身份改由仓库自身读回，不重跑 extraction。
- attempt 1（`.runs/phase-a-query-only-v1-attempt-1/`）失败并已冻结为 `response_model_content_absent`：provider 在请求固定 `response_format={"type":"json_object"}` 时返回 `message.content=null`，完整 payload 落在 `message.reasoning_content`（`finish_reason=stop`、`completion_tokens=219`、payload schema 有效），属于 response-model 合同缺陷，非 truncation 或 transport failure。隔离探针确认该字段为触发条件。失败 receipt SHA-256 `316c58f459f11b7676fe860214c8403a34a4c7d8482372611ad5659cb76d2eac`、mode `0444`；无 result artifact、无半成品事务，Query call `1`、L1/L2 call `0`、automatic write `0`。
- 修复绑定三个 red -> green 回归测试（`test_query_draft_request_does_not_pin_broken_json_object_format`、`test_query_draft_accepts_reasoning_only_payload`、`test_l1_producer_accepts_reasoning_only_payload`）：新增共享 `model_message_text()` 优先 `content`、回退 `reasoning_content`，对 `None`/空白/缺字段/非对象仍 fail-closed；并在 query-draft 与 L1/L2 producer 停止固定 `response_format`。schema 校验、`query_id` 回显、`fact`/`count` 限制与 evidence closure 均未放宽。
- attempt 2（`.runs/phase-a-query-only-v1-attempt-2/query-only-receipt.json`，mode `0444`）正式通过：`deepseek-v4-pro` requested/response model 一致，answer `memory:CoffeeBeverage`，evidence `evidence-turn-0000000000000001-user` 闭合到 `source-revision-5a316a782f8ab0b9bc006241`，`closure_complete=true`、`abstained=false`、`fallback_triggered=false`。authority SHA-256 `54e5cf48d4568adb813ac0da7a924222cdd700b55b0e310c13f4f9ecfa7625e8`、snapshot SHA-256 `bfba1a22e3ece021ddf407f36eb0c6f6b99680db47c76d8b6e391f7734a16e8d`、registry SHA-256 `e22b5dbf4560e3a2a63832ced01b388b8c7799b4445d6d6145ff6d4ef37f145d`；`query_call_count=1`、`l1/l2_producer_call_count=0`、`automatic_memory_write_count=0`、`deterministic_replay_verified=true`。执行后 v8 head、commit 数 `2` 与 raw artifact 均未漂移。
...
#### Phase A 第三、四轮评审与最终 attempt（2026-07-31）

- 第三轮返回 Critical `0`、Important `2`，均已核实为我自己 observer 实现的缺陷，修复提交 `a59b8ff`：write 观测窗口原本只覆盖 `recovery`/`snapshot`/`execution`，把「模型请求 + query compilation」这一唯一由不可信输出驱动代码的阶段留在观测之外，而 receipt 仍报 `automatic_memory_write_count=0`；`MUTATING_GIT_VERBS` 是 fail-open 允许清单，漏掉 `read-tree`（`git_memory_history.py:609-611` 在用，会改 index），任何新增 mutating verb 也会漏计。现新增 `compilation` 观测阶段，并反转为 `READ_ONLY_GIT_VERBS`，未识别 verb 一律计为写；verb 取 argv 首位，避免只读词作为参数掩盖 mutating verb。`status`/`diff` 因可写而不列入只读集合。
- 第四轮（迟到返回）再发现三项真实缺陷，修复提交 `90af662`：其一，attempt 4/5 是在 v1 schema 被原地修改期间写出的，带 `v1` 标签但已是测量版 `v2` 形状，**两个模型都无法加载**，而 AGENTS.md 当时正把 attempt 5 标为 Phase A 正式通过；因冻结字节不得编辑，改为新增 transitional 模型与 `load_query_only_receipt` 按标签+形状分派，八个 receipt 全部恢复可读。其二，`test_frozen_v1_receipts_still_validate` 只检查 `model_fields` 声明、从未校验任何冻结 receipt，这正是上一条缺陷能通过其所在提交自身验证的原因。其三，nesting guard 只作用于实例，两个 observer 交错后非 LIFO 退出会把 wrapper 永久留在类/模块上。
- 三个「测试证明不了自己名字所声称之事」的问题已修复，并用 mutation testing 验证：把 `l1_producer_call_count` 改回硬编码 `0` 时，原 19 个测试**全部通过**（证实评审判断），新增「窗口内强制构造 producer 并要求 guard 中止」的测试后该 mutation 转为失败；移除 nesting sentinel 失败 2 项，取消 verb filter 的 fail-closed 默认失败 3 项。restore 断言原先与 `_wrap` 工厂比较（窗口内也恒为真），现改为与真实类比较。
- Phase A 最终正式通过的是 attempt 10，HEAD `90af662`：`.runs/phase-a-query-only-v1-attempt-10/query-only-receipt.json`（mode `0444`，经 dispatcher 加载为 `OpenAIQueryOnlyReceiptV2`）。`observed_write_phases` 为 `recovery`/`snapshot`/`compilation`/`execution` 且与常量相等，四个计数为 `query=1`、`l1=0`、`l2=0`、`writes=0` 且全部来自测量，`deterministic_replay_verified=true`，requested/response 均为 `deepseek-v4-pro`、`attempts=1`，answer `memory:CoffeeBeverage` 未 abstain，evidence 闭合到 `source-revision-5a316a782f8ab0b9bc006241`；v8 head `0e04cd2c`、commit 数 `2`、raw `bfb6a284` 未漂移，credential scan `0`。focused 与相邻套件 `158 passed`。**先前把 attempt 5 记为正式通过的表述由本条更正**，attempt 5 的观测事实保留但不作为通过依据。
- 对抗性验证（每项均无 receipt 产出）：execution 层 abstention、response model 不一致、在 attempt 期间偷偷 `update-ref`、result 路径落在仓库内，四种违规全部 fail-closed。
- 仍然遗留并明确记录：`ExtractionCallObserver` 只统计构造、不统计既有实例的 `.produce()`，且只覆盖本模块全局名字绑定；checkpoint recovery 硬限制为单 checkpoint 历史，Phase C/D 若需要增量多 checkpoint 历史必须先解除该限制；attempt 4/5 的标签/形状不一致作为历史事实保留，仅通过 dispatcher 兼容读取。
