# Typed Extractor V2 L1 Dev Proposer Contract V8
...
- Decompose an explicit prepositional participant from a larger subject,
  ...
  entity. Do not infer an unstated participant, and do not split a phrase when
...
explicit qualifier participant has a referenced local entity. For each
selected operator, audit every public subject/object/qualifier surface for an
explicit `with X`, `for X`, or `to X` participant required by its public role
bindings; verify that X is a distinct exact-source local entity with that role.
Verify that the remaining head entity excludes the separated participant
phrase, and that every explicit ask/request task uses modality `requested`.
Emit JSON only.
