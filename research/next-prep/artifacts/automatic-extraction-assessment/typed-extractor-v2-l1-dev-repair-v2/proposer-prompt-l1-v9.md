# Typed Extractor V2 L1 Dev Proposer Contract V9
...
- Decompose an explicit prepositional participant from a larger subject,
  ...
  role. An explicit `with X` participant, `for X` beneficiary, or `to X`
  recipient must be a distinct exact-source local entity and must be bound to
  that operator role. The larger task/theme entity must use the exact head substring before the participant phrase
  when that head is the role referent;
  it must not retain the participant phrase after X is split into its own
  entity. After the split, that head entity must not contain ` with X`, ` for X`, or ` to X`.
  Do not infer an unstated participant, and do not split a phrase when
  the prepositional phrase is itself the semantic referent rather than a
  participant.
...
lifecycle, and operation references. Additionally verify: every selected
`likely_caused_by` proposal is emitted with modality `hypothetical`; every
public condition and scope value has exactly one matching binding; and every
explicit qualifier participant has a referenced local entity. Verify that each
local entity corresponding to the public candidate subject, object, or qualifier
uses that candidate surface rather than a source-turn pronoun or paraphrase.
For each selected operator, audit every public subject/object/qualifier surface
for an explicit `with X`, `for X`, or `to X` participant required by its public
role bindings; verify that X is a distinct exact-source local entity with that
role. Verify that the remaining head entity excludes the separated participant
phrase, does not still contain the matching prepositional suffix, and that every
explicit ask/request task uses modality `requested`.
Emit JSON only.
