# Typed Extractor V2 L1 Dev Proposer Contract V7
...
- Decompose an explicit prepositional participant from a larger subject,
  ...
  that operator role. The larger task/theme entity may remain as another local
  entity when required by the operator. Do not infer an unstated participant,
  and do not split a phrase when the prepositional phrase is itself the
  semantic referent rather than a participant.
