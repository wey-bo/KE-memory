# Typed Extractor V2 L2 Dev Proposer Contract V9
...
4. Select the L2 operator and abstraction for the resolved cross-turn meaning.
   Apply semantic dominance before coreference: repeated compatible supports
   that describe persistence of the same state and resolve to kind
   `long_running_state` require `state_summary`, even when the later wording
   refers back to the earlier state. Compatible multi-support preferences that
   resolve to kind `preference_profile` require `preference_aggregation`.
   Pronoun or definite-reference resolution inside either pattern does not
   make `coreference_resolution` dominant.
...
5. Map closure directly from abstraction: `lifecycle_resolution` requires
   ...
   require `multi_evidence_set` for this dev contract. `state_summary` and
   `preference_aggregation` also require `multi_evidence_set`.
...
- For every emission, copy `summary` exactly from
  ...
  Treat this as a literal field binding, not a semantic paraphrase. In
  particular, never substitute any typed L1 support predicate surface for
  `untyped_candidate.predicate`, even when both describe the same event.
...
- `kind`, abstraction method, closure pattern, predicate sense, canonical
  ...
  selected by meaning.
- Treat `operator_sense_bindings` as a closed pair catalog. Every structured
  claim must copy one exact `canonical_operator|sense` pair from that catalog;
  independently valid operator and sense values cannot be recombined. A
  resolved archive coreference claim uses
...
independently restarts contiguous entity numbering at `entity-01`. For every
emission, perform the literal assertion
`structured_claims[0].predicate.surface == untyped_candidate.predicate`; if it
is false, correct the claim before returning. Also verify every structured
claim's exact `canonical_operator|sense` pair occurs in the public
`operator_sense_bindings` catalog and re-run the abstraction dominance checks
before returning. Emit JSON only.
