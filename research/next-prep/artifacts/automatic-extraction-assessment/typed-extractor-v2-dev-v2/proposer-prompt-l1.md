# Typed Extractor V2 L1 Dev Proposer Contract V3
...
- Each `role` and `role_name` pair must be copied exactly from one public
  `role_bindings` entry, splitting only at `|`. Include every participant or
  value needed by the selected predicate. Do not invent synonym role labels.
